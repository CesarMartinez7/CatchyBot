...
def list_files(message):
    ...
    for file in descargas:
        bot.reply_to(
            message,
            f"Archivo: **{file.split('.')[0]}** Formato: **{file.split('.')[1]}**",
        )
...
def send_welcome(message) -> None:
    bot.reply_to(
        message,
        "Hello you 👋, soy tu bot asistente de descarga mp3, mp4 y webm hecho con Python  🤖. ",
    )
...
def help_fun(message) -> None:
    bot.reply_to(
        message,
        "Hola, soy tu bot asistente de descarga mp3, mp4 y webm hecho con Python  🤖. \n\n",
    )
...
def download_music(message):
    ...
    try:
        if len(url) > 43:
            bot.reply_to(
                message,
                "Demasiado Largo, no se pueden descargar formatos de radio o mix. 🌋",
            )
        elif len(url) == 43:
            ydl_optss = {
                "quiet": True,
